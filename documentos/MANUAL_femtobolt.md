# MANUAL COMPLETO — Cámara Orbbec Femto Bolt (iter 4)
...
> Para la **webcam global shutter**, usar `MANUAL_webcam.md`. Todo es igual salvo
> la cámara y su calibración (secciones 0–1 y el `--config`).
...
- Cámara **Orbbec Femto Bolt** conectada por USB 3.0.
...
- Distancia de trabajo cámara↔objeto: **50–70 cm** (la Femto Bolt tiene enfoque
  fijo 0.5–5 m; más cerca de 40 cm sale borroso).
...
**La Femto Bolt NO requiere calibración**: trae la calibración de fábrica en el
SDK. El archivo de config (`iter4\tracker_config.yaml`) tiene
`calibration_file:` vacío, lo que significa "usar la de fábrica". No hay que
...
Reemplazar los valores por los tuyos:
...
Montar el dodecaedro a 50–70 cm de la cámara, **fuera de la caja de luz**.
Ejecutar y, durante 90 segundos, **rotar el dodecaedro lentamente** mostrando
TODAS las caras (TOP, anillo superior, anillo inferior) en muchas orientaciones:

```powershell
python iter4\captura_calibracion.py --config iter4\tracker_config.yaml --duracion 90 --output iter4\data\captura_ba.npz
```
...
```powershell
python iter4\calibrar_tip_divot.py --config iter4\tracker_config.yaml --divot DOCK --plate-id 2 --plate-mm 59.6 --output-matriz iter4\data\StylusTipToDodecaedro_dock
```
...
texto **"Dodecaedro: N markers"** (N≥3). FPS estable ~17.
...
[ ] Tracker corriendo: "Dodecaedro: N markers" con N >= 3.
